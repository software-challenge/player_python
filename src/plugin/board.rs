...
impl Board {
    ...
    pub fn does_field_have_stream(&self, coords: &CubeCoordinates) -> bool {
        self.segment_with_index_at(*coords).is_some_and(|(i, s)| {
            let next_dir: CubeCoordinates = self.segments.get(i + 1).map_or_else(
                || self.next_direction.vector(),
                |s| s.direction.vector()
            );
            [
    ...
    pub fn is_sandbank(&self, coords: &CubeCoordinates) -> bool {
        self.get(coords).is_some_and(|field| field.field_type == FieldType::Sandbank)
    }
